...
def init_rankine_hugoniot(N,
    ...
    R_sp   = R_univ / molecular_weight  # J/(kg·K)
    p1     = p0_atm * 101325.0  # Pa
    rho1   = p1 / (R_sp * T0)
    a1     = np.sqrt(gamma * p1 / rho1)
    U1     = M * a1             # vitesse amont pour choc stationnaire
...
def hllc_flux(U_L, U_R, gamma=1.4):
    """
    Solveur de Riemann HLLC pour Euler 1D.
    Entrées :
      U_L, U_R : vecteurs conservatifs [rho, rho*u, E] à gauche et à droite
      gamma    : rapport des chaleurs spécifiques
    Sortie :
      F_hllc   : flux HLLC [rho*u, rho*u^2 + p, u*(E+p)]
    """
    ...
    # 2) Estimer les vitesses extrêmes S_L et S_R
    ...
    # 7) Sélection des cas HLLC par masque
    mask_L         = (S_L >= 0)
    mask_star_L    = (S_L < 0) & (S_star >= 0)
    mask_star_R    = (S_star < 0) & (S_R >= 0)
    mask_R         = (S_R < 0)

    # 8) Construction vectorielle du flux HLLC
...
# --- Terme convectif + visqueux + thermique ---
