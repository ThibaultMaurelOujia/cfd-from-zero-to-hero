...
def ssp_rk3_step(u, dt, dx, A, dA_dx, gamma=1.4):
    ...
    # On en déduit ρ_in via p = ρ R T
    rho_in = p_in / (R * T_in)
    rho_out= p_out / (R * T_out)
    
    # On choisit Mach_in (par exemple 0.5), puis u_in = Mach * c
    # Mach_in = 0.0
    # Mach_in = 0.001
    Mach_in = 0.1
...
def run_euler(name, scheme_func, params):
    ...
    # --- 2) Calcul de dx et de dt à partir de N et CFL ---
    dx = 1.0 / N
    ...
    # --- 3) Mise en place du plot (on trace A, p, T, M) ---
    x = np.linspace(0, 1, N, endpoint=False)
