...
void compute_hllc_flux(
    ...
    for(int i = N_ghost-1; i < Nx+N_ghost; ++i) {
        for(int j = N_ghost-1; j < Ny+N_ghost; ++j) {
            // 1) Passer en variables primitives
...
void compute_advection_weno3_HLLC(
    ...
    Field& conv_rho, Field& conv_rho_u, Field& conv_rho_v, Field& conv_E, // terme convectif  
    ...
    int Nx, int Ny, int N_ghost)
{
    ...
    // ----------------------- flux normal y ---------------------
    //  -> on echange (u,v) pour traiter v comme vitesse normale
    compute_hllc_flux(
        rho_Ly, rho_Ry,
        rho_v_Ly, rho_v_Ry,          // rho v  devient quantite de mvt normale
        rho_u_Ly, rho_u_Ry,          // rho u  devient quantite transverse
        E_Ly, E_Ry,
        flux_rho_y, flux_rho_v_y, flux_rho_u_y, flux_E_y,   // rho u <-> rho v dans la signature 
        gamma, Nx, Ny, N_ghost);
