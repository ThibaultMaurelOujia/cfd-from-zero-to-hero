...
void compute_vorticity(
    ...
    // differences centrales sur la zone interieure
...
void compute_schlieren(
    ...
    // derivees centrales sur la zone interieure
    #pragma omp parallel for collapse(2)
    for(int i = N_ghost; i < Nx+N_ghost; ++i) {
        for(int j = N_ghost; j < Ny+N_ghost; ++j) {
...
void compute_pressure(
    ...
    // calcul en chaque cellule interieure
    #pragma omp parallel for collapse(2)
    for(int i = N_ghost; i < Nx+N_ghost; ++i) {
        for(int j = N_ghost; j < Ny+N_ghost; ++j) {
            double r = rho[i][j];
...
void compute_viscous_stress_tensor(
    ...
    for(int i = N_ghost; i < Nx + N_ghost; ++i) {
        for(int j = N_ghost; j < Ny + N_ghost; ++j) {
            ...
            // du/dx, du/dy, dv/dx, dv/dy differences centrees 
