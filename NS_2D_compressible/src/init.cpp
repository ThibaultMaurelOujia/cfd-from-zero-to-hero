...
//  (rho,rho u,rho v,E) with (rho,u,v,p)
...
//---------------------------------------------------------------------------
void init_kelvin_helmholtz(       // profil symetrique + perturbation sinusoidale
    const SimulationParams& params,
    Field& rho, Field& rho_u, Field& rho_v, Field& E,
    double delta   = 0.005,      // epaisseur de couche de cisaillement
    double amp     = 1e-1,      // amplitude de la perturbation
    int    kx      = 4)         // mode sinusoidal en x
{
    ...
    double rho0 = params.rho_ref;       // densite uniforme de reference            1.0
    double p0   = params.p_ref;         // pression uniforme                        1.0
    const double a0   = sound_speed_ideal(gamma,p0,rho0);

    // Mach de cisaillement
    double M  = params.inflow_velocity;   
    double U0 = M * a0;                   // vitesse libre de cisaillement
    ...
    for(int i=N;i<Nx+N;++i){
        ...
        for(int j=N;j<Ny+N;++j){
            ...
            /* ---------------- primitives ---------------- */
            double u   = U0 * std::tanh((y-0.5*Ly)/delta);           // profil de cisaillement
...
//---------------------------------------------------------------------------
void init_one_x(                  // ecoulement uniforme en Mach = params.inflow_velocity
    const SimulationParams& params,
    Field& rho, Field& rho_u, Field& rho_v, Field& E)
{
    const int Nx=params.Nx, Ny=params.Ny, N=params.bc_ghost_layers;
    ...
    // Reference isotherme / isobare
    double rho0 = params.rho_ref;
    double p0   = params.p_ref;
    const double a0   = sound_speed_ideal(gamma,p0,rho0);

    double M  = params.inflow_velocity;       // interprete comme Mach
    double u0 = M * a0;
...
void init_sod_x(const SimulationParams& p,
                Field& rho,Field& rho_u,Field& rho_v,Field& E)
/* Discontinuite de Sod alignee en x */
{
    ...
    Field& rho_v, Field& E)
{
    ...
    const double U0     = 1.0;      // vitesse libre
    const double V0     = 0.0;
    const double beta   = 5.0;      // intensité
    const double x0     = 0.5 * Lx; // centre
    ...
    for(int i = N; i < Nx+N; ++i) {
        ...
        for(int j = N; j < Ny+N; ++j) {
            ...
            // perturbation de température / densité
            double T = 1.0
                - (gamma - 1.0)/(2.0*gamma)
            ...
            // conversion en variables conservées
            double ru, rv, Et;
