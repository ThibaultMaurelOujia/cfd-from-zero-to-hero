...
void init_one_x(const SimulationParams& params, Field& u, Field& v, Field& p){
    ...
    // On ne remplit que la zone interne [N_ghost..Nx+N_ghost[
    for (int i = N_ghost; i < Nx + N_ghost; ++i) {
        for (int j = N_ghost; j < Ny + N_ghost; ++j) {
            u[i][j] = U0;    // uniform inflow in x
            v[i][j] = 0.0;   // zero transverse velocity
            p[i][j] = 0.0;   // zero pressure
        }
    }
}
