...
void Simulator::time_loop() {
    ...
    while (t < params_.final_time) {
        ...
        // 1) compute dt based on CFL advective & diffusive
        ...
        // 2) one RK3 step: advection+diffusion -> projection
        step_SSP_RK3(dt);
...
void Simulator::step_SSP_RK3(double dt) {
    ...
    // --- stage 1 (predictor) ---
    //   u_stage = 0*u_old + 1*u_corr  (full_solve=false)
    rk3_substep(0.0, 1.0, dt);

    // --- stage 2 (first corrector) ---
    //   u_stage = 3/4*u_old + 1/4*u_corr  (full_solve=false)
    rk3_substep(3.0/4.0, 1.0/4.0, dt);

    // --- stage 3 (final corrector) ---
    //   u_stage = 1/3*u_old + 2/3*u_corr  (full_solve=true)
