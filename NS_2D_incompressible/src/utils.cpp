                        ...
                        int N_ghost)
{
    ...
    // On parcourt uniquement les cellules intérieures [N_ghost..Nx+N_ghost-1]×[N_ghost..Ny+N_ghost-1]
    for (int i = N_ghost; i < Nx + N_ghost; ++i) {
        for (int j = N_ghost; j < Ny + N_ghost; ++j) {
            double v = field[i][j];
...
void compute_vorticity(const Field& u, const Field& v, Field& w,
    ...
    // Calcul en différences centrales sur les cellules intérieures
    ...
    // Optionnel : mettre à zéro sur les ghost-layers
