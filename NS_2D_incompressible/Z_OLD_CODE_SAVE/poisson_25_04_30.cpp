...
// // ---------------------------------------------------------------------
// // mini_poisson.cpp   (g++ -O2 -std=c++17 mini_poisson.cpp)
// // ---------------------------------------------------------------------
// #include <Eigen/Dense>
...
// // ---------------------------------------------------------------------
// // deuxième membre b[i,j] = [[1,2,3],[4,5,6]]  (rangé en ordre (i,j))
// // ---------------------------------------------------------------------
// DenseMat make_rhs_dense()
// {
//     DenseMat rhs = DenseMat::Zero(N, 1);
//     double   data[Ny][Nx] = { {1,2,3}, {4,5,6} };

//     for (int j = 0, k = 0; j < Ny; ++j)      // j : y
//         for (int i = 0; i < Nx; ++i, ++k)    // i : x
//             rhs(k,0) = data[j][i];
...
// // ---------------------------------------------------------------------
// // assemble Laplacien périodique 2D  (plein ou creux)
// // ---------------------------------------------------------------------
// DenseMat make_laplacien_dense()
// {
//     DenseMat Tx = DenseMat::Zero(Nx,Nx);
//     DenseMat Ty = DenseMat::Zero(Ny,Ny);
...
//         Ty(j,(j-1+Ny)%Ny)       =  1.0;
//     }
//     DenseMat Ix = DenseMat::Identity(Nx,Nx);
//     DenseMat Iy = DenseMat::Identity(Ny,Ny);

//     DenseMat A = Eigen::kroneckerProduct(Tx,Iy)/(dx*dx)
//                + Eigen::kroneckerProduct(Ix,Ty)/(dy*dy);
...
// SpMat make_laplacien_sparse()
// {
//     std::vector<Eigen::Triplet<double>> trip;

//     auto idx = [](int i,int j){ return j*Nx + i; };      // (i,j) → linéaire

//     for (int j = 0; j < Ny; ++j){
//         for (int i = 0; i < Nx; ++i){
//             int k = idx(i,j);

//             auto add = [&](int ip,int jp,double c){
//                 trip.emplace_back(k, idx((ip+Nx)%Nx,(jp+Ny)%Ny), c);
//             };
//             add(i  , j  , -2./(dx*dx) - 2./(dy*dy));
//             add(i+1, j  ,  1./(dx*dx));
//             add(i-1, j  ,  1./(dx*dx));
//             add(i  , j+1,  1./(dy*dy));
//             add(i  , j-1,  1./(dy*dy));
//         }
//     }
//     SpMat A(N,N);   A.setFromTriplets(trip.begin(), trip.end());

//     std::cout << "\n--- Laplacien creux A  (nonZeros = " << A.nonZeros()
//               << ") -------------------------\n";
//     for (int k=0; k<std::min(8,N); ++k){
...
// // ---------------------------------------------------------------------
// // 1) résolution dense   (FullPivLU)
// // ---------------------------------------------------------------------
// void test_dense()
// {
//     std::cout << "\n=================  TEST DENSE  =====================\n";
//     DenseMat A   = make_laplacien_dense();
//     DenseMat rhs = make_rhs_dense();

//     DenseMat p = A.fullPivLu().solve(rhs);
...
// // ---------------------------------------------------------------------
// // 2) résolution creuse   (SparseLU)
// // ---------------------------------------------------------------------
// void test_sparse()
// {
//     std::cout << "\n=================  TEST SPARSE =====================\n";
//     SpMat    A   = make_laplacien_sparse();
//     DenseMat rhs = make_rhs_dense();
...
//         //
//     // 0) override pour test unitaire
//     //
//     constexpr int    testNx     = 3;
//     constexpr int    testNy     = 2;
//     constexpr int    testGhost  = 0;
//     constexpr double testLx     = 1.0;
//     constexpr double testLy     = 1.0;
//     constexpr double testDx     = testLx / testNx;  // = 1/3
//     constexpr double testDy     = testLy / testNy;  // = 1/2
    
//     // construction du Field b_test à partir de ton tableau de test
//     Field b_test(testNx + 2*testGhost,
//                  std::vector<double>(testNy + 2*testGhost, 0.0));
//     double data[ testNy ][ testNx ] = {{1,2,3},{4,5,6}};
//     for(int j=0; j<testNy; ++j)
//         for(int i=0; i<testNx; ++i)
//             b_test[i+testGhost][j+testGhost] = data[j][i];

//     // On remplace tous les usages de Nx,Ny,dx,dy,b par les versions de test :
//     Nx  = testNx;
//     Ny  = testNy;
//     N_ghost = testGhost;
//     dx  = testDx;
...
//     std::cout << "[Test] Nx=" << Nx << "  Ny=" << Ny
//               << "  dx=" << dx << "  dy=" << dy << "\n"
//               << "      RHS (vectorisé) = ";
//     for(int k=0; k<N; ++k) std::cout << rhs(k) << " ";
//     std::cout << "\n";
...
//     //     for(int i=0;i<Nx;++i){
//     //         T.emplace_back(i,i,-2.0);
//     //         T.emplace_back(i,(i+1)%Nx, 1.0);
//     //         T.emplace_back(i,(i-1+Nx)%Nx, 1.0);
//     //     }
//     //     Tx.setFromTriplets(T.begin(),T.end());
//     // }
//     // // --- Ty ---
...
//     //     for(int j=0;j<Ny;++j){
//     //         T.emplace_back(j,j,-2.0);
//     //         T.emplace_back(j,(j+1)%Ny, 1.0);
//     //         T.emplace_back(j,(j-1+Ny)%Ny, 1.0);
//     //     }
//     //     Ty.setFromTriplets(T.begin(),T.end());
//     // }
//     // --- Tx ---
...
// for(int j=0,k=0;j<Ny;++j)
//     for(int i=0;i<Nx;++i,++k)
//         p_new[i+N_ghost][j+N_ghost] = sol(k);
// std::cout << "\n--- Solution p_new (sans ghost) ---\n";
// for(int j=0,k=0;j<Ny;++j) {
//     for(int i=0;i<Nx;++i,++k) {
//     std::cout << std::setw(10) << p_new[i][j];
//   }
//   std::cout << "\n";
// }
